<template>
  <div class="app-container">
            ...
            <div class="card-header">
              ...
              <div class="header-right">
                <el-button type="primary" :icon="Edit" link @click="handleUpdate(item)"
                  v-hasPermi="['system:enterpriseConfig:edit']">修改</el-button>
                <el-button type="primary" :icon="Refresh" link @click="handleSyncToken(item)"
                  v-hasPermi="['system:enterpriseConfig:edit']">同步token</el-button>
              </div>
            </div>
          </template>
          <div class="card-body">
              ...
              <el-row :gutter="10" class="mb8">
                ...
                <el-col :span="1.5">
                  <el-button type="success" plain icon="Edit" :disabled="single"
                    @click="handleUpdateRule">修改</el-button>
                </el-col>
                <el-col :span="1.5">
                ...
                <el-col :span="1.5">
                  <el-button type="info" plain icon="InfoFilled" @click="getLatestControl">查询最新管控</el-button>
                </el-col>
                <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
              </el-row>
                ...
                <el-collapse-item title="最新管控措施" name="1">
                  <div class="control-info">
                    <el-descriptions :column="2" border>
                      ...
                      <el-descriptions-item label="管控时间" :span="2">{{ controlInfo.startTime }} 至 {{ controlInfo.endTime
                        }}</el-descriptions-item>
    ...
    <transition name="dialog-fade">
      <div v-if="dyQrcodeDialogVisible" class="custom-dialog-overlay" @click.self="closeDyQRCodeDialog">
        <div class="custom-dialog">
          <div class="dialog-header">
            <span>入口二维码</span>
            <i class="el-icon-close" @click="closeDyQRCodeDialog"></i>
          </div>
          <div class="dialog-body">
            <div class="qrcode-wrapper">
              <img :src="dyQrcode" alt="QR Code" class="qrcode-image" />
              <div class="download-button" @click="downloadDyQRCode">
                <i class="el-icon-download"></i>
                <span>下载二维码</span>
              </div>
            </div>
    ...
    <transition name="dialog-fade">
      <div v-if="supplementDialogVisible" class="custom-dialog-overlay" @click.self="closeSupplementDialog">
        <div class="custom-dialog">
          <div class="dialog-header">
            <span>货物信息补录二维码</span>
            <i class="el-icon-close" @click="closeSupplementDialog"></i>
          </div>
          <div class="dialog-body">
            <div class="qrcode-wrapper">
              <img :src="supplementQr" alt="QR Code" class="qrcode-image" />
              <div class="download-button" @click="downloadSupplementQRCode">
                <i class="el-icon-download"></i>
                <span>下载二维码</span>
              </div>
            </div>
      ...
      <el-form ref="ruleRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="禁行时间段">
          <div v-for="(time, index) in form.forbiddenTimes" :key="index" class="time-range">
            <el-time-picker v-model="time.start" format="HH:mm" value-format="HH:mm:ss" placeholder="开始时间" />
            <span class="time-separator">至</span>
            <el-time-picker v-model="time.end" format="HH:mm" value-format="HH:mm:ss" placeholder="结束时间" />
            <el-button type="danger" icon="Delete" circle @click="removeForbiddenTime(index)" size="small" />
        ...
        <el-form-item v-for="(item, index) in selectItems" :key="index" :label="item.label" :prop="item.prop">
          <el-select v-model="form[item.prop]" multiple clearable collapse-tags :placeholder="item.placeholder"
            popper-class="custom-header" :max-collapse-tags="7">
            <template #header>
...
const data = reactive({
  ...
  fuelTypeDict: [],
  vehicleClassDict: [],
  emissionStandardDict: [],
});

const { queryParams, formData, rules } = toRefs(data);
const { qrcode, qrcodeDialogVisible } = toRefs(data);
const { dyQrcode, dyQrcodeDialogVisible } = toRefs(data);
const { supplementQr, supplementDialogVisible } = toRefs(data);
const { ruleList, ruleOpen } = toRefs(data);
  ...
  proxy.$refs["enterpriseConfigRef"].validate(valid => {
    if (valid) {
      if (formData.value.id != null) {
        updateEnterpriseConfig(formData.value).then(() => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addEnterpriseConfig(formData.value).then(() => {
...
function getDyQRCode(row) {
  ...
  currentCompany.value = row;
  const companyId = row.companyId;
  getDynamicQrCode(companyId).then(response => {
    if (!response) {
      console.error("No QR code data returned");
      return;
    }
    const blob = new Blob([response], { type: 'image/png' });
    data.dyQrcode = URL.createObjectURL(blob);
    data.dyQrcodeDialogVisible = true;
  });
}

// 获取补录二维码
function getSupplementQr(row) {
  if (!row || !row.companyId) {
    console.error("Invalid row or missing companyId");
    return;
  }
  currentCompany.value = row;
  const companyId = row.companyId;
  getSupplementQrCode(companyId).then(response => {
    if (!response) {
      console.error("No QR code data returned");
      return;
    }
    const blob = new Blob([response], { type: 'image/png' });
    data.supplementQr = URL.createObjectURL(blob);
    data.supplementDialogVisible = true;
  });
}
...
// 关闭dyQrcodeDialog弹窗
function closeDyQRCodeDialog() {
  data.dyQrcodeDialogVisible = false;
  currentCompany.value = null;
}

// 关闭supplementDialog弹窗
function closeSupplementDialog() {
  data.supplementDialogVisible = false;
  currentCompany.value = null;
}
...
function downloadQRCode() {
  ...
  const companyName = getCompanyName(currentCompany.value.companyId);
  const link = document.createElement('a');
  link.href = qrcode.value;
  link.download = `${companyName}场外车辆信息录入二维码.png`;
...
function downloadDyQRCode() {
  ...
  const companyName = getCompanyName(currentCompany.value.companyId);
  const link = document.createElement('a');
  link.href = dyQrcode.value;
  link.download = `${companyName}入口二维码.png`;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
}

function downloadSupplementQRCode() {
  if (!currentCompany.value) {
    console.error("No company selected");
    return;
  }
  const companyName = getCompanyName(currentCompany.value.companyId);
  const link = document.createElement('a');
  link.href = supplementQr.value;
  link.download = `${companyName}货物信息补录二维码.png`;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
}
...
// 打开全屏对话框
function openFullscreenDialog(item) {
  currentCompany.value = item;
  currentCompanyName.value = getCompanyName(item.companyId);
...
function getRuleList() {
  ...
  listForbiddenRule(params).then(response => {
  ...
  proxy.$refs["ruleRef"].validate(valid => {
    if (valid) {
      ...
      if (form.value.id != null) {
        ...
        form.value.companyId = currentCompany.value?.companyId;
...
function getLatestControl() {
  queryLatestControl(currentCompany.value?.companyId).then(response => {
...
/** 同步企业token */
function handleSyncToken(row) {
  if (!row || !row.companyId) {
    proxy.$modal.msgError("请选择要同步的企业");
    return;
  }

  syncConfig(row.companyId)
    .then(response => {
      proxy.$modal.msgSuccess(`"${getCompanyName(row.companyId)}"同步token成功`);
      console.log(`同步companyId ${row.companyId} token成功`);
    })
    .catch(error => {
      proxy.$modal.msgError(`"${getCompanyName(row.companyId)}"同步token失败`);
      console.error(`同步companyId ${row.companyId} token失败:`, error);
    });
}
